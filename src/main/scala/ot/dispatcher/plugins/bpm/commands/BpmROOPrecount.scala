...
case class BpmROOPrecount(keywords: Map[String, String], searchId: Int, utils: PluginUtils) {
...
//    val W_row = Window.orderBy(timestampField, "sort_column")
    ...
    val description = full_description.groupBy("ROO").agg(collect_set(col("array1_distinct_events")).as("distinct_events"), collect_set(col("array1_distinct_relations")).as("distinct_relations"), collect_list(col("caseId")).as("array_cases"))
      .withColumn("full_actions", array_distinct(flatten(col("distinct_events"))))
      .withColumn("full_relations", array_distinct(flatten(col("distinct_relations"))))
      .withColumn("full_cases", array_distinct(col("array_cases")))
...
object BpmROOPrecount extends ApplyModel {
  ...
  override def apply(modelName: String, modelConfig: Option[Config], searchId: Int, featureCols: List[String],
    ...
    BpmROOPrecount(keywords, searchId, utils).transform
  }
}
