...
case class BpmPrecount(keywords: Map[String, String], searchId: Int, utils: PluginUtils) {

  import utils._

  val DEF_COLUMN: String = "action"

  def transform(df: DataFrame): DataFrame = {
    val column_to_group = Caster.safeCast[String](
      keywords.get("trace_field"),
      DEF_COLUMN,
      sendError(searchId, "The value of parameter 'version' should be of string type")
    )
...
//    val W_row = Window.orderBy(timestampField, "sort_column")
    ...
    val full_input = full_input1.unionByName(fakeStart2)
      .withColumn("relation", struct("eventId", "next_event"))
      .orderBy("_time")
...
//      .withColumn("full_cases", array_distinct(col("array_cases")))
    ...
    val description = full_description.groupBy(column_to_group).agg(collect_set(col("array1_distinct_events")).as("distinct_events"), collect_set(col("array1_distinct_relations")).as("distinct_relations"), collect_list(col("caseId")).as("array_cases"))
      .withColumn("full_actions", array_distinct(flatten(col("distinct_events"))))
      .withColumn("full_relations", array_distinct(flatten(col("distinct_relations"))))
      .withColumn("full_cases", array_distinct(col("array_cases")))
      .drop("distinct_events", "distinct_relations", "array_cases")
      .select("eventId", "full_actions")
      .limit(1)
...
object BpmPrecount extends ApplyModel {
  ...
  override def apply(modelName: String, modelConfig: Option[Config], searchId: Int, featureCols: List[String],
    ...
    BpmPrecount(keywords, searchId, utils).transform
  }
}
